...
# TEST XXX
# filter out unneeded columns
df_total = df_total.filter(['Solar  - Actual Aggregated [MW]','Hour'])

for ii in range(0,24):
    print(ii)
    df_temp =  df_total[df_total['Hour'] ==ii]
    print(df_temp['Solar  - Actual Aggregated [MW]'].sum())
    
for ii in [22,23,0,1,2,3,4]:
    df_temp2 = df_total[(df_total['Hour'] ==ii) & (df_total['Solar  - Actual Aggregated [MW]'] != 0)]
    df_temp2.to_csv('test_out' + str(ii)+'.csv')
